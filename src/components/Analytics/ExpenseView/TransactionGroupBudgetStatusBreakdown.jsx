...
/**
 * Transaction Group Budget Status Breakdown Component
 * Displays expense distribution by transaction group with budget status colors
 ...
 */
const TransactionGroupBudgetStatusBreakdown = ({ expenseData, activeBudget, formatCurrency }) => {
  ...
  /**
   * Process expense data for transaction group budget status chart visualization
   */
  const processChartData = () => {
    ...
    // Process groups with percentages and budget status colors
    const processedData = Object.entries(groupTotals)
      .map(([groupId, actualSpent]) => {
        ...
        return {
          ...
          color: getBudgetStatusColor({ actualSpent, budgetAmount, hasBudget, variance, variancePercentage }), // Use budget status colors
  ...
  /**
   * Get budget status color based on performance
   */
  const getBudgetStatusColor = (group) => {
    if (!group.hasBudget) return '#9ca3af'; // Gray for no budget
    
    if (group.variance > 0) return '#dc2626'; // Red for over budget
    if (group.variancePercentage > -10) return '#d97706'; // Orange for close to budget
    return '#16a34a'; // Green for within budget
  };
  ...
  if (!chartData || chartData.length === 0) {
    return (
      <div className="transaction-group-budget-breakdown">
        <div className="chart-header">
          <h3>Transaction Group Budget Status</h3>
          <p className="chart-subtitle">Budget performance by transaction group</p>
        </div>
  ...
  return (
    <div className="transaction-group-budget-breakdown">
      ...
      <div className="chart-content">
        ...
        {/* Transaction Group Budget Legend */}
        <div className="transaction-group-budget-legend">
          {chartData.map((group, index) => (
            <div
              key={group.groupId}
      ...
      {selectedGroup && (
        <div className="selected-group-details">
          <h4>{selectedGroup.label} Budget Analysis</h4>
          ...
          {selectedGroup.hasBudget && (
            <>
              ...
              <div className="detail-row">
                ...
                <span className={`status-${selectedGroup.budgetStatus}`}>
                  {selectedGroup.budgetStatus === 'over-budget' && 'Over Budget'}
                  {selectedGroup.budgetStatus === 'close-to-budget' && 'Close to Budget'}
                  {selectedGroup.budgetStatus === 'within-budget' && 'Within Budget'}
                </span>
              </div>
            </>
          )}
          {!selectedGroup.hasBudget && (
            <div className="detail-row">
              <span>Budget Status:</span>
              <span className="status-no-budget">No Budget Set</span>
            </div>
          )}
          <div className="detail-row">
...
export default TransactionGroupBudgetStatusBreakdown;